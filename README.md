...
| 원본 | 대상 |
|---|---|
| PDF | PNG / JPEG / WebP (페이지별, 해상도·크기·품질 선택) + 문서 정리(병합/분할/회전/삭제/순서) |
| 이미지(PNG/JPG/WebP/BMP/GIF/SVG/AVIF/HEIC/TIFF/ICO) | PDF(여러 장 합치기), PNG/JPEG/WebP/BMP(같은 포맷 재저장 허용), ICO(멀티사이즈), SVG(벡터화) |

- GIF/AVIF는 **입력 전용**, HEIC/TIFF는 추가 시점에 PNG로 디코드. BMP/ICO 출력은 자체 인코더.
- 이미지 공통 옵션: 크기(px), 품질(jpeg/webp), 회전(90°)·좌우/상하 반전·흑백, **자르기**,
  **배경 제거**(흰색→투명 / AI 누끼 — 완전 오프라인, 모델 번들), 워터마크. 전부 미리보기 실시간 반영.
- Ctrl+V 클립보드 붙여넣기, 실행취소/다시실행(Ctrl+Z/Y), 진행 로딩바.
- ⚠️ AI 배경 제거: `@imgly/background-removal` **1.4.5 고정**(모델 npm 배포가 1.4.5까지 — 신버전은 CDN 전용이라 오프라인 불가).
  모델(~354MB)은 electron-builder extraResources(`bgrm-data`)로 번들, main의 `bgrm://` 프로토콜로 서빙.

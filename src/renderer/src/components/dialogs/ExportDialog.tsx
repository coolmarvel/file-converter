...
import { bytesToUrl } from '../../convert/image'
...
export default function ExportDialog({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const url = bytesToUrl(s.bytes, s.mime)
        setResult((prev) => {
  ...
  return (
    ...
    >
      ...
      <Note ok={!error}>{error ?? (ready ? `${humanSize(result.size)} · ${dims(result.w, result.h)}px · 실제 인코딩 결과 (선택 파일 기준)` : '인코딩 중…')}</Note>
    </ClassicDialog>
  )
}

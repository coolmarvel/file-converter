...
export interface ImageSizeResult {
  ...
  height: number | null
  dpi: number
  /** 비율 잠금 — 여러 파일일 때 가로만 지정해 각자 비율 유지 */
  lock: boolean
}
  ...
  return (
    ...
    >
      <Box className="tnum" sx={{ color: color.textSecondary }}>
        현재: {src.width.toLocaleString()} × {src.height.toLocaleString()} 픽셀{fileCount > 1 ? ` · 선택 파일 기준 (파일 ${fileCount}개에 적용)` : ''}
      </Box>
      ...
      <GroupBox title="해상도">
        ...
        <Box sx={{ fontSize: font.xs, color: color.textSecondary }}>
          {st.resample
            ? '픽셀을 다시 계산합니다. 크게 줄일 때는 2배씩 단계적으로 줄여 선명하게 만듭니다.'
            : '인쇄 크기와 해상도만 바뀌고 픽셀은 그대로입니다. PNG·JPEG 파일과 이미지→PDF 페이지 크기에 반영됩니다.'}
        </Box>
      </GroupBox>
